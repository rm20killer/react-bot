...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (message.member.roles.cache.find(r => r.name === modid) || message.member.roles.cache.find(r => r.name === adminid) || message.member.roles.cache.find(r => r.id === helper)) {
      ...
      const channels1 = await message.guild.channels.cache.filter(channel => channel.type === 'GUILD_TEXT');
      ...
      for (const channel of channels1) {
        ...
        if (channel[1].parentId === "629695220065239063"||channel.parentID === "716754944472121516") {
          ...
          channel1.permissionOverwrites.edit(memberRole, { SEND_MESSAGES: null });
