...
module.exports = {
    ...
    async execute(message, args, client) {
        if (message.member.roles.cache.find(r => r.name === adminid)) {
            ...
            //send file to user
            const attachment = await new Discord.MessageAttachment(`./temp/rawdata.json`, `RawData-${target.id}.json`);
            message.reply({content: `Here is the raw data for ${target.user.tag}`, files: [attachment]});
