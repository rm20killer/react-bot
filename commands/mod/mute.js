...
const tempmute = async function (
  ...
  const embed2 = new Discord.MessageEmbed().setDescription(
    `<@${targetmember.user.id}> has been muted for ${timeString}`
  );
...
const mute = async function (message, client, targetmember, reason, expires) {
  ...
  channel.send({ embeds: [embed] });
  const embed2 = new Discord.MessageEmbed().setDescription(
    `<@${targetmember.user.id}> has been muted`
  );
  message.channel.send({ embeds: [embed2] });
...
function DMUser(targetmember, message, reason) {
  ...
  const embed3 = new Discord.MessageEmbed().setDescription(
    `You were muted in Gamers React for ${reason}`
  );
...
function DMUser(targetmember, message, reason, time) {
  ...
  const embed3 = new Discord.MessageEmbed().setDescription(
    `You were tempmuted  in Gamers React for ${time}| ${reason}`
  );
  targetmember.send({ embeds: [embed3] }).catch((error) => {
    message.channel.send(`Could not dm ${targetmember.user.tag}`);
    console.log(error);
