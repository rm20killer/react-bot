...
module.exports = {
  ...
  async execute(message, args) {
    if (
      ...
      message.member.roles.cache.find((r) => r.id === "838238288216588358")
    ) {
      ...
      if (setgame === "youtube") {
        ...
        return message.channel.send({
          content:
            "no game selected, games include: `youtube`,`betrayal`,`fishing`,`lettertile`,`wordsnack`,`doodlecrew`, `Spellcast` and `chess`",
        });
      }
      let channel = message.member.voice.channel;
      if (!channel)
        return message.channel.send({
          content: "You have to be connected in a voice call to use that!",
        });
      ...
      fetch(`https://discord.com/api/v8/channels/${channel.id}/invites`, {
        ...
        .then((body) => {
          return message.channel.send(
            `https://discord.gg/${body.code} (Click The Link To Setup!)`
          );
        })
        .catch((err) => {
          console.log(err);
