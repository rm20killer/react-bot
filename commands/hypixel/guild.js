    ...
    hypixel
      ...
      .then(async (guild) => {
        const guildInfoEmbed = new Discord.MessageEmbed(base)
          ...
          .addField(
            "Members",
            `\`${Object.keys(guild.members).length}/125\``,
            true
          )
          .addField(
            "Date Created",
            `<t:${Math.ceil(guild.createdAtTimestamp / 1000)}:R>`,
            true
          );
        var arr = [];
        for (i = 0; i < guild.expHistory.length; i++) {
          var exp = `\`•\` ${guild.expHistory[i].day}: \`${commaNumber(
            JSON.stringify(guild.expHistory[i].exp)
          )}\``;
          arr.push(exp);
        }
        guildInfoEmbed.addField(
          "GEXP History",
          `${arr[0]}\n${arr[1]}\n${arr[2]}\n${arr[3]}\n${arr[4]}\n${arr[5]}\n${arr[6]}\n`
        );
      ...
      .catch((e) => {
        ...
        if (e.message === errors.GUILD_DOES_NOT_EXIST) {
          const guild404 = new Discord.MessageEmbed(base)
            .setAuthor("Error", "https://i.imgur.com/OuoECfX.jpeg")
            .setDescription(
              "I could not find that guild in the API. Check spelling and name history."
            );
          message.reply({
            embeds: [guild404],
            allowedMentions: { repliedUser: false },
