...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const embed = new Discord.MessageEmbed()
      ...
      .setColor(2374108)
      .setDescription(
        `If you need help with the minecraft server ask on the Gamers React Minecraft Discord or website`
      )
      .addField("IP:", "`play.gamersreact.net`")
      .addField(
        "Version:",
        "Premium Java only, 1.16.5 with support from 1.8 to 1.18.1"
      );
    //message.channel.send({ embeds: [embed] })
    message.channel
      .send({ embeds: [embed], components: [row23] })
      .catch(console.error);
  },
};
