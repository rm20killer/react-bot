...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        let index = database.gameArray.findIndex(
        ...
        if (status === "accept") {
            if (interaction.user.id === game.player2) {
                ...
                await interaction.reply({ content: "You are not player 2", ephemeral: true })
            }
        }
        //console.log(game);
...
async function playClicked(interaction, game, embed, database, index) {
    ...
    if (square === 0) {
        ...
        if (winner === 1 || winner === 2 || winner === 3) {
            ...
            await interaction.update({ embeds: [embed], components: generateBoard(rows, game) });
        }
        let dataJSON = JSON.stringify(database, null, 2);
        fs.writeFileSync(`./utils/data/tictactoe.json`, dataJSON);
