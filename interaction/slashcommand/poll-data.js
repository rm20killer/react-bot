...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName("voteid").setDescription("vote id").setRequired(true)
    ),
  ...
  async execute(interaction, client) {
    ...
    if (verifyID(voteID, guildId) === false) {
      ...
      let data = JSON.parse(
        fs.readFileSync(`./utils/data/vote/${guildId}/${voteID}.json`)
      );
      ...
      await interaction.reply({
        ...
        files: [
          {
            attachment: `./utils/data/vote/${guildId}/${voteID}.json`,
            name: `votedata.json`,
          },
        ],
      });
