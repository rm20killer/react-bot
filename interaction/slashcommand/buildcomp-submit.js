...
module.exports = {
  ...
  async execute(interaction, client) {
    return interaction.reply({
      content:
        "You can't submit a build comp! Just yet, wait for the <#629695330153267219> when you can.",
      ephemeral: true,
    });
...
async function NewUser(interaction, buildcompSubmitsData) {
  ...
  fs.writeFileSync(
    "./utils/data/buildcompSubmits.json",
    JSON.stringify(buildcompSubmitsData, null, 2)
  );
  const embed = new Discord.MessageEmbed()
    .setTitle("Build Comp image uploaded")
    .setDescription(
      `Your build comp has been submitted for ${interaction.options.data[0].value}`
    )
...
async function NonNewUser(
  interaction,
  buildcompSubmitsData,
  userData,
  indexForUser
) {
  ...
  if (submissionExists) {
    ...
    submissions[submissionIndex].screenshot =
      interaction.options.data[1].attachment.url;
    ...
    const embed = new Discord.MessageEmbed()
      ...
      .setDescription(
        `Your build comp has been submitted for ${interaction.options.data[0].value} has been updated with`
      )
      .setImage(interaction.options.data[1].attachment.url)
  ...
  if (submissionsLength === 9) {
    ...
    const embed = new Discord.MessageEmbed()
      ...
      .setDescription(
        "You have submitted all required images for your build comp!\nGet full list of your entries here with /buildcomp-data"
      );
