  ...
  async execute(client, oldMember, newMember) {
    ...
    if (newMember.pending === false) {
      if (oldMember.pending === false) { }
        ...
        try {newMember.roles.add(memberRole).catch((error) => {console.log(error);});
        newMember.roles.add(pronoun).catch((error) => {console.log(error);});
        newMember.roles.add(notification).catch((error) => {console.log(error);});
        newMember.roles.add(level).catch((error) => {console.log(error);});
        } catch (error) {
          console.log(error);
          return;
        }
    ...
    if (newMember.guild.id === "629695220065239061") {
      ...
      if (!shadRole && shasRole) {
        newMember.guild.channels.cache.get("841018811657355354")
          .send("<@" + newMember.id + "> has gotten into a Gamers React video.");
        return;
      }
      //Server Booster
      if (!hadRole && hasRole) {
        newMember.guild.channels.cache.get("788078716546318418").send(`${boostemote} <@${newMember.id}> boosted the server\nThank you 💗`);
        newMember.guild.channels.cache.get("629695330153267219").send(`${boostemote} <@${newMember.id}> boosted the server\nThank you 💗`);
      ...
      if (isSpecialBool === true) {
        ...
        if (newMember.roles.cache.find((r) => r.id === special)) {
          ...
          newMember.roles.add(special).catch((error) => {console.log(error);});
          return;
        }
      } else {
        if (newMember.roles.cache.find((r) => r.id === special)) {
          newMember.roles.remove(special).catch((error) => {console.log(error);});
        }
      }
    }
    return;
