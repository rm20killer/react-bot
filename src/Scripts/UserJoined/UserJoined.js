...
module.exports = {
  async userjoined(member, client) {
    ...
    //report
    channel3 = client.channels.cache.find((channel) => channel.id === "892816609712930836");
    ...
    if (sus.arr.includes(name2)) {
      ...
      let BadNameEmbed = new Discord.MessageEmbed().setDescription(`You been kicked for using a banned name on Gamers React.`);
      try {
        member.send({ embeds: [BadNameEmbed] }).catch((error) => {console.log(`could not dm user ${name}`);});
      } catch (error) {
        console.log(error);
        return;
      }
