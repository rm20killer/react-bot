  ...
  async execute(message, args) {
    ...
    hypixel
      ...
      .then(async (player) => {
        ...
        if (player.guild !== null && player.guild.tag !== null) {
          embed.setTitle(
            `[${player.rank}] ${player.nickname} [${player.guild.tag}]`
          );
          embed.addField(
            "Guild",
            `\`${player.guild.name} [${player.guild.tag}]\``
          );
        }
        ...
        embed.addField(
          "First Login",
          `<t:${Math.ceil(player.firstLoginTimestamp / 1000)}:R>`
        );
        embed.addField(
          "Last Login",
          `<t:${Math.ceil(player.lastLoginTimestamp / 1000)}:R>`
        );
